...
export default function Page() {
  ...
  const { data: attr } = useSWR(`${API_BASE}/api/timeseries/attribution?metric=${metric}&frequency=weekly`, fetcher);
  const { data: geo } = useSWR(`${API_BASE}/api/geo/activity`, fetcher);

  // date→evidence
  const evidenceMap = useMemo(() => {
    const m = new Map<string, any[]>();
    if (attr?.attributions) {
      for (const e of attr.attributions) {
        const k = new Date(e.date).toISOString().slice(0, 10);
        m.set(k, e.reasons ?? e.evidence ?? []);
      }
    }
    if (ts?.series) {
  ...
  useEffect(() => {
    if (len === 0) return;
    const width = WINDOW_POINTS[mode] === Infinity ? len : Math.min(WINDOW_POINTS[mode], len);
    const start = WINDOW_POINTS[mode] === Infinity ? 0 : Math.max(0, len - width);
    const end = len - 1;
  ...
  const step = (dir: -1 | 1) => {
    ...
    if (dir === -1) {
      // 過去へ：新しい右端 = 旧ウィンドウの左端の「ひとつ前」
      ne = s - 1;
      ns = ne - (width - 1);
    } else {
      // 先へ：新しい左端 = 旧ウィンドウの右端の「ひとつ後」
      ns = e + 1;
      ne = ns + (width - 1);
    }
        ...
        <div className="lg:col-span-2 rounded-2xl shadow-soft p-5 bg-surface-200 border border-brand-900/40">
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart
                key={`${idxRange?.[0]}-${idxRange?.[1]}`}           // ← 再描画安定化
                data={chartData}
          ...
          <div className="mt-4">
            ...
            <div className="max-h-56 overflow-auto rounded-xl border border-brand-900/40 bg-surface-100">
              <table className="w-full text-[12px]">
                ...
                <tbody>
                  {chartData.slice(-12).map((d, i, arr) => {
